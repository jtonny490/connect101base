...
import { bounties, bountySubmissions, createBounty, createBountySubmission, createCvEntry, createDeal, createMilestone, createPayment, createSubmission, getBounty, getBountySubmission, getCvForUser, getDeal, getMilestone, getPayment, updateDeal, updateMilestone, updatePayment, users, submissions, payments, milestones, } from './store.js';
const app = express();
...
app.post('/api/milestones/:id/submissions', (req, res) => {
    ...
    if (!previewUrl)
        return res.status(400).json({ error: 'Missing previewUrl' });
...
app.post('/api/bounty-submissions/:id/accept', (req, res) => {
    ...
    if (!bounty)
        return res.status(404).json({ error: 'Bounty not found' });
    if (bounty.status !== 'open')
        return res.status(400).json({ error: 'Bounty is already closed' });
...
function estimateLocalAmount(amountSats, currency) {
    const rateMap = {
        KES: 0.23,
        UGX: 0.34,
        NGN: 2.5,
